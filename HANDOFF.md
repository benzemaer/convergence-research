...
R2A-T04 status: score_scope_repair_pending_review
...
authorization_revision: 2_superseded_before_formal_run
...
current_stop: R2A-T04 Score-only scope repair review
```

Repair commit 的精确 HEAD 与 Quality 成功后，才允许创建 metadata-only authorization revision 3。Revision 3 的精确 HEAD Quality 成功前，不得开始正式运行。本轮不读取 accepted Score 数据，不创建 formal output root，也不消费 formal attempt。
...
Revision 2 的 HEAD `9d3c2dab43a10b12931db921ef730db6e8552ff1` 在正式运行开始前被替代，未被使用，也未消费 formal attempt。替代理由仅为 `scope_corrected_to_score_parameter_response_and_interval_structure`，不表示 Score release、evaluator、panel 或 benchmark 失败。
...
1. `refactor: scope R2A-T04 to Score response audit`：实现 Score-only harness、schema、tests 和文档，配置保持 `score_scope_repair_pending_review`、`formal_run_authorized=false`。Push 后等待精确 repair HEAD Quality success。
2. `chore: authorize R2A-T04 Score response audit`：只修改 config、config schema 与状态文档，把 `reviewed_harness_head` 绑定到精确 repair HEAD，设置 revision 3 和 `authorized_not_started`。不得修改 `src/**`、`scripts/**` 或 `tests/**`。Push 后等待精确 authorization HEAD Quality success。
