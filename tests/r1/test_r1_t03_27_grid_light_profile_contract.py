...
class R1T03LightProfileContractTest(unittest.TestCase):
    ...
    def test_zero_confirmed_interval_marked_input_fact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = _run_fixture(root, fixture)
            self.assertEqual(
                summary["confirmed_interval_input_status"],
    ...
    def test_nonzero_confirmed_interval_is_computed_input_fact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            fixture = _write_fixture(root, with_intervals=True)
            summary = _run_fixture(root, fixture)
            self.assertEqual(
                summary["confirmed_interval_input_status"],
                "nonzero_confirmed_input_fact",
            )
            self.assertEqual(summary["confirmed_interval_row_count_total"], 27)
            profile = _load_json(
                root / summary["output_paths"]["profile_by_config_state_json"]["path"]
            )
            by_state = {row["state_name"]: row for row in profile[:4]}
            self.assertEqual(by_state["S_P"]["confirmed_interval_status"], "computed")
            self.assertEqual(
                by_state["S_PC"]["confirmed_interval_status"],
                "no_confirmed_interval_for_state",
            )
...
def _write_fixture(
    root: Path, all_raw_false: bool = False, with_intervals: bool = False
) -> dict[str, Path]:
    paths = {
    ...
    manifest = _full_grid_manifest(root, all_raw_false, with_intervals)
    ...
    summary = {
        ...
        "counts": {"confirmed_interval_row_count_total": 27 if with_intervals else 0},
    }
...
def _full_grid_manifest(
    root: Path, all_raw_false: bool, with_intervals: bool
) -> dict[str, object]:
    candidates = []
    artifacts = {}
        ...
        for q_label, q in (("10", 0.1), ("20", 0.2), ("30", 0.3)):
            for k in (2, 3, 5):
                ...
                artifacts[config_id] = _write_config_artifacts(
                    root, config_id, w, q, k, all_raw_false, with_intervals
                )
    return {
...
def _write_evidence(root: Path, summary: dict[str, object]) -> Path:
    ...
    fields = {
        ...
        "confirmed_interval_input_status": summary["confirmed_interval_input_status"],
        "confirmed_interval_row_count_total": summary[
