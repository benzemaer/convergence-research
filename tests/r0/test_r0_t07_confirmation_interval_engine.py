...
def result_for(
    results, day: int, state_name: str, k: int, security_id: str = "000001.SZ"
):
    trading_date = f"2026-{day:04d}"
...
class R0T07ConfirmationIntervalEngineTest(unittest.TestCase):
    def test_streak_confirmation_and_no_backfill_for_k_values(self) -> None:
        rows = [daily_state(1), daily_state(2), daily_state(3)]
        results = compute_daily_confirmations(rows)

        day1_k1 = result_for(results, 1, "S_PCVT", 1)
        self.assertEqual(day1_k1.raw_streak, 1)
        self.assertTrue(day1_k1.confirmed_state)
        self.assertEqual(day1_k1.confirmation_start_date, "2026-0001")
        self.assertEqual(day1_k1.confirmation_date, "2026-0001")
    ...
    def test_confirmed_nested_invariant_and_raw_violation_guard(self) -> None:
        ...
        blocked = compute_daily_confirmations([invalid], confirmation_k_values=(1,))
        for item in blocked:
    ...
    def test_invalid_k_values_are_rejected(self) -> None:
        with self.assertRaises(ValueError):
            compute_daily_confirmations([daily_state(1)], confirmation_k_values=(0,))
        with self.assertRaises(ValueError):
            compute_daily_confirmations([daily_state(1)], confirmation_k_values=(4,))
