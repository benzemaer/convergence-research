...
def _formal_binding(repo: Path, commit: str, rel: str) -> dict[str, Any]:
    ...
    status = subprocess.run(["git", "diff", "--quiet", "--", rel], cwd=repo).returncode
...
def _check_startup(repo: Path, commit: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, expected in required.items():
        if handoff.get(key) != expected or validation.get(key) != expected:
            raise R2T05Blocked(
                "startup_status=blocked_missing_authoritative_t04_final_gate_binding"
            )
...
def _check_freeze_plan(
    config: dict[str, Any], startup: dict[str, Any]
) -> dict[str, Any]:
    ...
    if isinstance(actual, list):
        for version in actual:
            if (
                "planned_state_version_id" not in version
                or "state_version_id" in version
            ):
    ...
    expected_by_cell = {
        version["source_candidate_cell_id"]: version for version in expected
    }
    selected_units = [
        unit for unit in units if unit.get("primary_disposition") == "selected"
    ]
    rejected_units = [
        unit for unit in units if unit.get("primary_disposition") == "rejected"
    ]
    ...
    for version in expected:
        matches = [
            ...
            if unit.get("primary_candidate_cell_id")
        ...
        if (
            ...
            or unit.get("shared_candidate_cell_id")
    ...
    if set(unit.get("primary_candidate_cell_id") for unit in selected_units) != set(
        expected_by_cell
    ):
        raise R2T05Blocked("t04_freeze_decision_additional_selected_candidate")
    exclusions = {
        "W250_materialized_version_count": sum(
            version.get("W") != 120 for version in actual
        ),
        ...
        "additional_selected_candidate_count": sum(
            version.get("source_candidate_cell_id") not in expected_by_cell
            for version in actual
        ),
...
def _build_event_map(
    ...
    columns = [
        ...
        "primary_route_id",
        "strict_core_route_id",
    ]
    con.execute("DROP TABLE t05_selected_versions")
    con.execute(
        "CREATE TEMP TABLE t05_selected_versions AS SELECT * FROM (VALUES "
        + ",".join(["(" + ",".join("?" for _ in columns) + ")"] * len(route_rows))
        + ") AS x("
        + ",".join(columns)
        + ")",
        [value for row in route_rows for value in row],
    )
    events = con.execute(
        """
    ...
    con.execute(
        "CREATE TEMP TABLE t05_event_map AS SELECT * FROM (VALUES "
        + ",".join(["(" + ",".join("?" for _ in range(10)) + ")"] * len(mapped))
        + ") AS x(state_version_id,source_candidate_cell_id,source_scan_event_id,security_id,first_component_id,first_component_start_date,first_qualification_time,canonical_event_id,identity_payload,identity_payload_sha256)",
        [
            value
            for row in mapped
            for value in [
...
def _materialize_membership(con: duckdb.DuckDBPyConnection) -> dict[str, int]:
    ...
    before = dict(
        con.execute(
            "SELECT state_version_id,count(*) FROM r2_canonical_event_membership GROUP BY 1"
        ).fetchall()
    )
    con.execute(
        """
    ...
    return {state: after.get(state, 0) - before.get(state, 0) for state in after}
...
def _materialize_daily(con: duckdb.DuckDBPyConnection, source_run_id: str) -> None:
    ...
    con.execute(
        ...
        SELECT d.state_version_id,d.state_line,d.window_track_id,d.security_id,d.trade_date,
               ...
               d.qualified_event_risk_set_eligible,s.strict_core_member,d.quality_state,
               d.candidate_config_id,?
    ...
    qualified_mismatch = con.execute(
        """
        SELECT count(*) FROM r2_canonical_daily_state
        WHERE qualified_event_risk_set_eligible
          AND (NOT state_risk_set_eligible OR NOT confirmed_state)
        """
    ).fetchone()[0]
    if qualified_mismatch:
        raise R2T05Error(
            f"canonical_daily_qualified_event_risk_formula_failure:{qualified_mismatch}"
        )
    active_fk_violation = con.execute(
        """
        SELECT count(*)
        FROM r2_canonical_daily_state d
        LEFT JOIN r2_canonical_event_zone e
          ON e.state_version_id=d.state_version_id
         AND e.security_id=d.security_id
         AND e.event_id=d.active_event_id_as_of
        WHERE d.active_event_id_as_of IS NOT NULL AND e.event_id IS NULL
        """
    ).fetchone()[0]
    if active_fk_violation:
        raise R2T05Error(
...
def _compact_audit_statuses(run_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for name in COMPACT_AUDIT_STATUS_FILES:
        path = run_dir / name
        if not path.is_file():
            statuses[name] = {"status": "failed", "failure": "missing"}
            continue
        if path.suffix == ".json":
            document = json.loads(path.read_text(encoding="utf-8"))
            statuses[name] = {"status": document.get("status")}
            continue
...
def _table_fingerprint(con: duckdb.DuckDBPyConnection, table: str) -> dict[str, Any]:
    ...
    return {
        ...
        "columns": columns,
        "stable_multiset_sha256": fingerprint,
    }
...
def _collect_reports(
    ...
    freeze: dict[str, Any],
    source_run_id: str,
    terminal_added: dict[str, int],
    expected_daily_keys: int,
) -> dict[str, Any]:
    versions = [
        row[0]
        for row in con.execute(
    ...
    daily_rows = con.execute("""
      SELECT state_version_id,count(*) daily_rows,
        ...
        sum(CASE WHEN qualified_event_risk_set_eligible THEN 1 ELSE 0 END) qualified_event_risk_rows,
    ...
    _write_csv(
        ...
        [
            {
                "state_version_id": row[0],
                "expected_daily_rows": expected_daily_keys,
                "actual_daily_rows": row[1],
                "eligible_rows": row[2],
                "valid_rows": row[3],
                "raw_true_rows": row[4],
                "confirmed_rows": row[5],
    ...
    source_event_counts = dict(
        con.execute(
            """
            SELECT v.state_version_id,count(e.scan_event_id)
            FROM t05_selected_versions v
            JOIN src.event_zone e ON e.candidate_cell_id=v.source_candidate_cell_id
            GROUP BY 1
            """
        ).fetchall()
    )
    _write_csv(
        run_dir / "r2_t05_event_reconciliation.csv",
        [
            {
                "state_version_id": row[0],
                "source_event_rows": source_event_counts.get(row[0], 0),
                "canonical_event_rows": row[1],
                "security_count": row[2],
                "component_count": row[3],
                "bridge_count": row[4],
                "bridged_gap_days": row[5],
                "confirmed_days": row[6],
                "trading_span": row[7],
                "density_min": row[8],
                "density_max": row[9],
                "revision_min": row[10],
                "revision_max": row[11],
                "status": "passed"
                if source_event_counts.get(row[0], 0) == row[1]
                else "failed",
            }
    ...
    _write_csv(
        run_dir / "r2_t05_membership_reconciliation.csv",
        [
            {
                "state_version_id": row[0],
                "canonical_membership_rows": row[1],
                "event_member_rows": row[2],
                "retrospective_component_rows": row[3],
                "prequalification_rows": row[4],
                "bridge_rows": row[5],
                "unqualified_reentry_rows": row[6],
                "qualified_risk_rows": row[7],
                "availability_lag_min": row[8],
                "availability_lag_max": row[9],
                "terminal_rows_added_total": terminal_added.get(row[0], 0),
                "status": "passed",
            }
        ...
        [
            "state_version_id",
            "canonical_membership_rows",
            "event_member_rows",
            "retrospective_component_rows",
            "prequalification_rows",
            "bridge_rows",
            "unqualified_reentry_rows",
            "qualified_risk_rows",
            "availability_lag_min",
            "availability_lag_max",
            "terminal_rows_added_total",
            "status",
        ],
    )
    _write_csv(
        run_dir / "r2_t05_strict_core_reconciliation.csv",
        [
            {
                "state_version_id": row[0],
                "primary_confirmed_rows": row[1],
                "strict_core_rows": row[2],
                "subset_violations": row[3],
                "status": "passed" if row[3] == 0 else "failed",
            }
            for row in con.execute("""
    ...
    _write_csv(
        run_dir / "r2_t05_availability_time_audit.csv",
        [
            {
                "state_version_id": row[0],
                "daily_rows": row[1],
                "membership_rows": row[2],
                "availability_order_violations": row[3],
                "terminal_rows_added": terminal_added.get(row[0], 0),
                "status": "passed" if row[3] == 0 else "failed",
            }
            for row in con.execute("""
        ...
        [
            ...
            "daily_rows",
            "membership_rows",
            "availability_order_violations",
            "terminal_rows_added",
            "status",
        ],
    )
    _write_csv(
        run_dir / "r2_t05_risk_set_audit.csv",
        [
            {
                "state_version_id": row[0],
                "state_risk_rows": row[1],
                "qualified_risk_rows": row[2],
                "event_member_not_state_risk": row[3],
                "bridge_risk_rows": row[4],
                "prequalification_risk_rows": row[5],
                "unqualified_reentry_risk_rows": row[6],
                "qualified_not_state_risk_rows": row[7],
                "status": "passed"
          ...
          SELECT state_version_id,
            ...
            sum(CASE WHEN is_unqualified_reentry_day AND qualified_event_risk_set_eligible THEN 1 ELSE 0 END),
            sum(CASE WHEN qualified_event_risk_set_eligible AND NOT state_risk_set_eligible THEN 1 ELSE 0 END)
        ...
        [
            ...
            "state_risk_rows",
            "qualified_risk_rows",
            "event_member_not_state_risk",
            "bridge_risk_rows",
            "prequalification_risk_rows",
            "unqualified_reentry_risk_rows",
            "qualified_not_state_risk_rows",
            "status",
        ],
    )
    _write_csv(
        run_dir / "r2_t05_exit_censor_reason_profile.csv",
        [
            {
                "state_version_id": row[0],
                "zone_status": row[1],
                "exit_reason": row[2],
                "event_count": row[3],
                "right_censored_count": row[4],
                "quality_break_count": row[5],
            }
            for row in con.execute("""
    ...
    fingerprints = {
        table: _table_fingerprint(con, table)
        for table in PUBLIC_TABLES + ["r2_t05_event_id_lineage"]
    }
    _write_json(
        run_dir / "r2_t05_table_fingerprint.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "database_path": str(
                (run_dir / config["output"]["database_name"]).relative_to(ROOT)
            ).replace("\\", "/"),
            "tables": fingerprints,
        },
    )
    ...
    for version in config["selected_versions"]:
        selected_reconciliation.append(
            {
                "source_candidate_cell_id": version["source_candidate_cell_id"],
                "state_version_id": version["state_version_id"],
                "selected": True,
                "W": version["W"],
                "d": version["d"],
                "g": version["g"],
                "source_event_count": source_counts.get(version["state_version_id"], 0),
                "status": "passed",
            }
        )
    _write_csv(
        run_dir / "r2_t05_selected_cell_reconciliation.csv",
        selected_reconciliation,
        [
            "source_candidate_cell_id",
            "state_version_id",
    ...
    _write_csv(
        run_dir / "r2_t05_version_materialization_map.csv",
        [
            {
                "state_version_id": v["state_version_id"],
                "state_line": v["state_line"],
                "window_track_id": v["window_track_id"],
                "source_candidate_cell_id": v["source_candidate_cell_id"],
                "strict_core_source_candidate_cell_id": v[
                    "strict_core_source_candidate_cell_id"
                ],
                "W": v["W"],
                "K": v["K"],
                ...
                "d": v["d"],
                "g": v["g"],
                "status": "materialized_author_stage",
            }
    ...
    lineage_cursor = con.execute(
        "SELECT * FROM r2_t05_event_id_lineage ORDER BY state_version_id,security_id,source_scan_event_id"
    )
    ...
    startup_report = {
        key: value for key, value in startup.items() if key != "bound_artifacts"
    }
    _write_json(
        run_dir / "r2_t05_source_readiness.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "status": "passed",
            "startup": startup_report,
            "freeze_plan_status": freeze["freeze_plan"].get("freeze_plan_status"),
            "selected_version_count": 2,
            "strict_core_only_count": 2,
            "source_database": source_info,
            "required_source_tables": sorted(SOURCE_TABLES),
        },
    )
    strict_fail = con.execute(
        "SELECT count(*) FROM r2_canonical_daily_state WHERE strict_core_member AND NOT confirmed_state"
    ).fetchone()[0]
    risk_fail = con.execute(
        "SELECT count(*) FROM r2_canonical_event_membership WHERE (is_bridged_gap OR is_prequalification_confirmed_day OR is_unqualified_reentry_day OR qualified_event_risk_set_eligible AND NOT state_risk_set_eligible) AND qualified_event_risk_set_eligible"
    ).fetchone()[0]
    ...
    """).fetchone()[0]
    audit_statuses = _compact_audit_statuses(run_dir)
    anomalies = []
    if len(versions) != 2:
    ...
    if con.execute(
        "SELECT count(*) FROM r2_canonical_event_zone WHERE zone_status='FINALIZED' AND exit_reason='quality_break'"
    ).fetchone()[0]:
        anomalies.append("quality_break_natural_exit_conflict")
    for name, audit in audit_statuses.items():
        if audit.get("status") != "passed":
            anomalies.append(f"compact_audit_failed:{name}")
    anomaly = {
        "task_id": "R2-T05",
        "run_id": run_dir.name,
        "status": "passed" if not anomalies else "failed",
        "blocking_failure_count": len(anomalies),
        "anomalies": anomalies,
        "compact_audit_statuses": audit_statuses,
...
def _artifact_entries(
    repo: Path, run_dir: Path, config: dict[str, Any]
) -> list[dict[str, Any]]:
    entries = []
    ...
    if database.is_file():
        entries.append(
            {
                "path": database.relative_to(repo).as_posix(),
                "sha256": _sha256_file(database),
                "size_bytes": database.stat().st_size,
                "lfs_policy": "local_not_committed_due_size_policy",
            }
        )
...
def _write_packages(
    ...
    _write_json(
        run_dir / "r2_t05_independent_validation.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "status": "pending",
    ...
    manifest = {
        ...
        "artifacts": entries,
        "database_path": (run_dir / config["output"]["database_name"])
        .relative_to(repo)
        .as_posix(),
        "database_sha256": next(
            (x["sha256"] for x in entries if x["path"].endswith(".duckdb")),
            "",
        ),
        "source_run_id": source_run_id,
        ...
        "worker_count": 1,
        "status": "author_stage_package",
    }
    _write_json(run_dir / "r2_t05_output_manifest.json", manifest)
    package = {
        "task_id": "R2-T05",
        "run_id": run_dir.name,
        "execution_status": "executed_and_validated_pending_independent_scientific_review",
        "formal_task_completed": False,
        "scientific_review_status": "pending_independent_scientific_review",
        "repository_final_gate_status": "pending_independent_scientific_review_and_exact_head_validation",
        "R2-T06_allowed_to_start": False,
        "R2-T07_allowed_to_start": False,
        "R3_allowed_to_start": False,
        "selected_version_count": 2,
        "strict_core_only_count": 2,
        "W250_materialized_version_count": 0,
        "shared_q_independent_state_version_count": 0,
        "PCT_parent_product_count": 0,
        "startup_handoff_status": "passed",
        "source_run_id": source_run_id,
        "execution_code_commit": execution_commit,
        ...
        "anomaly_scan_status": reports["anomaly"]["status"],
        "compact_audit_statuses": reports["audit_statuses"],
        "independent_validation_status": "pending",
        "result_analysis_path": (run_dir / "r2_t05_result_analysis.md")
        .relative_to(repo)
        .as_posix(),
        "scope_note": "selected-only canonical materialization; T06/T07/T08/R3 excluded",
    }
    _write_json(run_dir / "r2_t05_result_package.json", package)
    _write_json(
        run_dir / "r2_t05_experiment_summary.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "stage": "R2",
    ...
    _write_json(
        run_dir / "r2_t05_author_stage_scientific_review.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "scientific_review_status": "pending_independent_scientific_review",
            ...
            "review_required": "independent_scientific_review_bound_to_exact_tested_head",
        },
    )
    _write_json(
        run_dir / "r2_t05_committed_artifact_validation.json",
        {
            "task_id": "R2-T05",
            "run_id": run_dir.name,
            "status": "pending_commit",
            "formal_task_completed": False,
            "validation_mode": "awaiting_generated_artifact_commit",
        },
    )
...
def run_formal(config_path: Path, output_dir: Path, repo: Path = ROOT) -> Path:
    ...
    t03_package = _json_from_commit(
        repo, execution_commit, config["inputs"]["t03_result_package_path"]
    )
    database, source_info = _source_hash_and_tables(
        repo,
        execution_commit,
        t03_package,
        config["inputs"]["t03_event_zone_scan_path"],
    )
    ...
    input_binding = {
        **input_bindings,
        "t03_database": source_info,
        "startup": startup_report,
        "freeze_plan_hash": freeze["bound_input_bindings"][
            config["inputs"]["t04_freeze_plan_path"]
    ...
    con.execute(
        "CREATE TEMP TABLE t05_source_cell_execution AS SELECT * FROM src.cell_registry WHERE false"
    )
    for version in versions:
        source_cell = con.execute(
            "SELECT * FROM src.cell_registry WHERE candidate_cell_id=?",
            [version["source_candidate_cell_id"]],
        ).fetchall()
        ...
        row = source_cell[0]
        if (
            row[3] != version["state_line"]
            or row[4] != version["W"]
            or row[5] != version["K"]
            or row[6:10] != (version["qP"], version["qC"], version["qT"], version["qV"])
            or row[10] != version["d"]
            or row[11] != version["g"]
        ):
    ...
    _build_event_map(con, config, versions, source_run_id)
    ...
    if con.execute(
        "SELECT count(*) FROM r2_canonical_daily_state WHERE state_version_id LIKE '%W250%'"
    ).fetchone()[0]:
