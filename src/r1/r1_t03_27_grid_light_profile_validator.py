...
def _check_summary(
    ...
    interval_status = summary.get("confirmed_interval_input_status")
    interval_total = summary.get("confirmed_interval_row_count_total")
    if interval_status not in {
        "zero_confirmed_input_fact",
        "nonzero_confirmed_input_fact",
    }:
        errors.append("summary_confirmed_interval_input_status_mismatch")
    elif interval_status == "zero_confirmed_input_fact":
        if interval_total != 0:
            errors.append("summary_confirmed_interval_total_mismatch")
        if summary.get("zero_confirmed_interval_acknowledged") is not True:
            errors.append("summary_zero_confirmed_not_acknowledged")
    elif not isinstance(interval_total, int) or interval_total <= 0:
        errors.append("summary_confirmed_interval_total_mismatch")
...
def _check_evidence(
    ...
    if evidence.get("confirmed_interval_input_status") != str(
        summary.get("confirmed_interval_input_status")
    ):
        errors.append("evidence_confirmed_interval_input_status_mismatch")
    if evidence.get("confirmed_interval_row_count_total") != str(
        summary.get("confirmed_interval_row_count_total")
    ):
        errors.append("evidence_confirmed_interval_total_mismatch")
