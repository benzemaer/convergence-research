...
def _write_anchor_target_status(con: Any, path: Path) -> None:
    ...
    a AS (
      SELECT transition_path, path_order, W, q,
        sum(P_ONSET AND lag_k=1)::BIGINT AS anchor_event_count,
        sum(P_ONSET AND lag_k=1 AND target_anchor_valid)::BIGINT AS target_valid_at_anchor_count,
        sum(P_ONSET AND lag_k=1 AND target_anchor_valid AND target_anchor_raw IS TRUE)::BIGINT AS target_already_active_at_anchor_count,
        sum(P_ONSET AND lag_k=1 AND target_anchor_valid AND target_anchor_raw IS FALSE)::BIGINT AS target_inactive_at_anchor_count,
