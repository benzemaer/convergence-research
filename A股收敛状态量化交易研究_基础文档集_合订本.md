...
### 12.2 两阶段推进（GOV-T02 双 PR 推进与生效边界）

GOV-T02 合并后，从下一项尚未启动的 R3-R6 formal experiment 开始，正式实验固定采用两个相互独立的 PR。已经执行过的 R1/R2 formal experiment 不迁移、不重跑、不补写新门禁记录；历史治理与 R2-T02 专用证据继续作为历史证据保留。

Implementation PR 只提交代码、CLI、config、schema、runner、validator、tests 和 task doc，不执行 formal run，不提交正式结果，不要求 SCIENTIFIC PASS。Implementation PR 通过普通工程 CI 和代码审阅后合并到 `main`。Formal-result PR 必须绑定 implementation merge SHA，在合入后的代码上执行 formal run，提交 evidence、hash、row count、coverage、compact artifacts、submission manifest 和作者结果分析；它不得修改 implementation protected paths。

没有真实运行 evidence 不得把 task 标记 completed；没有 generic formal-result validator 和独立 SCIENTIFIC PASS 不得推进 README 或下游 gate。代码完成不等于研究完成，artifact 生成完成也不等于下游授权完成。
...
工作流顺序为：Implementation PR 提交代码、runner、task-specific validator、tests 和 runbook；Implementation PR 合并到 `main` 后，Formal-result PR 绑定 merge SHA 并执行 formal run；随后提交结果包、anomaly scan 和作者 result analysis；Formal-result PR 保持 draft，`scientific_review_status = pending` 且 gate=false；独立 reviewer 审核实际 artifacts 和报告；根据 review 修复或补充分析；提交 scientific review record；手动运行 generic formal-result gate；gate passed 后才转 Ready、合并并推进 README 和下游 gate。Codex 不得在独立科学审阅后自行跳过 generic final gate。

### 12.15 GOV-T02 双 PR 门禁绑定

Formal-result submission manifest 必须列出实际 `result_package`、`result_analysis`、`input_manifest`、implementation protected paths、scientific-review protected paths 和允许的审阅后治理路径。SCIENTIFIC PASS marker 必须绑定 task ID、run ID、artifact commit 和 result package SHA-256，reviewer 必须不同于 implementation actor。review commit 不要求等于当前 PR head，只要求是当前 PR head 的 ancestor；full profile 则必须绑定当前 PR head，并且只验证当前 head 的工程完整性，不替代 SCIENTIFIC PASS。
