...
### 12.2 两阶段推进（GOV-T02 当前流程）

GOV-T02 将 R3-R6 新任务固定为两个逻辑阶段，但两个阶段不等于必须两个 PR。默认优先同一个 PR 分两次提交：先提交 implementation，用户审阅并明确批准 implementation SHA，再基于该 SHA 执行 formal run，追加结果并由用户审阅。任务复杂、运行时间长或需要分离代码与结果时，才使用两个独立 PR。
...
formal run 的必要条件是代码 commit 已被用户明确审阅通过，不是 PR 已合并。用户批准记录至少包含：
...
Implementation 阶段必须明确 `formal_run_allowed: false`、`formal_run_status: not_started`、`result_review_status: not_started` 和 `readme_advanced: false`。Implementation PR 或同一 PR 的 implementation commit 不得提交正式运行结果、正式结果 analysis、正式结果 manifest、基于正式结果的科学结论或 README 下一任务推进。

若用户批准后、formal run 前修改了实现、配置、schema、研究参数、统计定义、输入选择、runner 或 validator，原批准自动失效，必须提交新的 implementation commit 并重新请求审阅。纯 PR body 修改不影响批准；formal run 后的结果、analysis 和 README 修改也不反向改变已运行代码。

GOV-T01 的复杂 formal package、SCIENTIFIC PASS、独立 reviewer、repository final gate 和 handoff 规则仅作为已完成 R1/R2 研究的 legacy workflow 保留，不适用于 GOV-T02 管辖的新任务；新 R3-R6 task 不得依赖、调用或复制这些历史 gate。
...
每个 formal experiment 必须提交 clean-checkout 可审核的结果包，至少包含 `experiment_summary.json`、`primary_results.csv` 或小型 JSON、`diagnostic_summary.json`、`anomaly_scan.json`、`engineering_validation_result.json`、`result_analysis.md`、formal evidence、`scientific_review.json` 和 `scientific_review.md`。小型 committed artifact 必须使用 repo-relative path、通过 SHA-256 复核，并能在 Git clean checkout 中读取；大型行级 DuckDB/Parquet 不提交，但必须记录 path、sha256、schema、row_count、security_count、date_min/date_max 和 input manifest，且不得冒充 committed result artifact。
...
### 12.14 Draft → 用户审阅 → 结果决定流程

GOV-T02 新任务不使用自动 scientific gate、SCIENTIFIC PASS、repository final gate 或 Draft/Ready 特殊条件。Implementation 审阅通过后，任务作者记录 `reviewed_implementation_sha` 和 `formal_run_allowed: true`，再执行 formal run。Formal-result 阶段提交 compact result summary、primary results、manifest、hash、result analysis、validator 输出和大型本地产物元数据；不强制全局结果包 schema。

用户直接决定结果：接受时记录 `result_review_status: accepted`、`next_task_allowed: true`，更新 README、将当前 task 标记 completed 并推进下一 task；需要修订时记录 `result_review_status: needs_revision`、`next_task_allowed: false`，不得推进 README。实现错误必须返回 Implementation 阶段并重新审阅；输入或运行参数变化必须重新执行 formal run；旧结果不得支持新 implementation SHA。
