...
in_progress via PR #41；stage 1 contract/local-schema-probe complete，stage 2 candidate materialization plan / fallback repair probe framework added，stage 3 local formal candidate artifact materialization added。
...
第三阶段在同一 PR 分支追加 HiThink raw market prices formal candidate artifact materialization。Stage 3 读取显式传入的 HiThink raw K parquet 和 stage 1 probe summary report，生成 `d1.raw_market_prices` formal candidate artifact、candidate quality summary、candidate materialization report 和 file hash summary。Stage 3 不发布 accepted source，不发布 `data_version`，不写 DuckDB，不生成 D3 artifact，不解锁 R0。
...
- 建立 Stage 3 local candidate artifact 契约与 materializer，在显式授权的本地输出目录生成 ignored candidate artifact 与质量摘要。
...
- `configs/d2/hithink_raw_market_prices_candidate_artifact_contract.v1.json`
...
- `scripts/materialize_d2_hithink_raw_market_prices_candidate.py`
...
- `tests/test_d2_hithink_raw_market_prices_candidate_artifact_contract.py`
- `tests/test_probe_hithink_raw_ohlcv_schema.py`
- `tests/test_build_d2_hithink_candidate_materialization_plan.py`
- `tests/test_materialize_d2_hithink_raw_market_prices_candidate.py`
...
## Stage 3 Candidate Artifact Boundary

Stage 3 materializer 只读取显式传入的 HiThink raw K file、Stage 1 probe summary report、security mapping JSON 和已提交 contracts。`--raw-k-path` 可以显式指向本地 HiThink parquet；`--probe-report` 和 `--security-mapping` 不得指向 `data/raw`、`data/external`、MarketDB、parquet、DuckDB 或 `.day` 文件。`--output-dir` 可指向 ignored 的 `data/generated/d2/d2_t09_candidate_raw_market_prices/`，不得指向 `data/raw` 或 `data/external`。

Stage 3 candidate row artifact 只允许包含 `d1.raw_market_prices` 的 17 个目标字段。`source_symbol`、`thscode`、`ts_code`、`code`、vendor payload、raw rows、qfq/hfq rows、future return、label、PCVT、backtest 或 portfolio 字段不得写入 candidate rows。源代码到 `security_id` 的映射只能通过显式 `--security-mapping` 中 `mapping_status = accepted` 的行完成；未映射源代码被 drop 并计数。

`observed_at` 必须来自显式 `--source-observed-at`，不得从 `trading_date` 推导。缺失 `trading_status` 或 `price_limit_status` 时写入 `unknown` 并在 quality summary 中计数；不得静默写成 `normal_trading`、`none`、`false` 或 `0`。

Stage 3 生成的是 local formal candidate artifact，不是 accepted formal source，不是 accepted formal dataset，不发布 `data_version`，不写 DuckDB，不创建 accepted manifest，不生成 D2 adjusted price / D3 artifact / R0 state。
...
- Stage 3：HiThink raw market prices formal candidate artifact materialization；只生成 ignored local candidate artifact、quality summary、materialization report 和 file hash summary，不发布 accepted dataset。
