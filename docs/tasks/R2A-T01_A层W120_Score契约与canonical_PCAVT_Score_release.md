...
`3f36357be9d469d7a9751eef79f368676d7ec97a`。唯一一次 formal run 已由 execution
commit `7c3fe76c575eb350a8e94d2f7534d123e865a64c` 完成，当前状态为
`formal_result_review`。Owner 已批准 execution amendment，不要求 successor formal run；
formal review evidence bundle 已准备，但仍不授权 `DONE`、T01 acceptance、README gate
advancement 或 R2A-T02。
...
Implementation review、formal authorization commit、精确 execution commit Quality、唯一一次
正式物化、独立 validator、实际 result analysis、独立 formal-result review 与 Owner 验收均已完成。
当前状态记录为：
...
```text
formal_run_id: R2A-T01-20260718T103110891Z
formal_authorization_id: R2A-T01-FORMAL-AUTH-20260718T103110891Z
score_release_id: pcavt-score-w120-v1-c7e04f11a2cd09aa
score_release_preimage_sha256: c7e04f11a2cd09aaa1a26bf0288ceed79f168d78968a2277d1e44f9514c206bf
worker_count: 16 spawn worker processes
validator_status: passed
validator_failed_checks: none
analysis_status: passed
release_recommendation: publish_candidate; subsequently owner-accepted
blocking_anomalies: none
DONE: present
```

Formal review evidence bundle 位于：

```text
data/generated/r2a/r2a_t01/R2A-T01-20260718T103110891Z/formal-review/
```

其中 `manifest.json`、`schema.json`、`validation_receipt.json` 与
`result_analysis.md` 是原 formal package 的 byte-identical copy；
...
`passed`，recommendation 为 `accept_candidate`，全部 mismatch 为 0。Owner 已据此正式接受
唯一 formal run 与 canonical Score release。Accepted handoff 位于
`data/generated/r2a/r2a_t01/R2A-T01-20260718T103110891Z/r2a_t01_accepted_result_handoff.json`，
同一 accepted result 根目录包含唯一 canonical `DONE`。

本次只注册 A-layer W120 Score contract 与 canonical PCAVT Score release；未创建动态阈值
q/K、连续确认、daily convergence state、interval、d/g、预测、回测或交易信号。
R2A-T02 输入在 accepted handoff 中标记为 eligible，但在 PR #109 合并前不得启动；当前
`R2A-T02_started=false`。

跨项目只允许交付 `score_data.duckdb`、`manifest.json`、`schema.json` 和
`r2a_t01_accepted_result_handoff.json`。接收方必须验证 Score release ID、Score database
SHA-256、schema SHA-256 与 accepted handoff status。该交付是 canonical PCAVT W120 Score
interface，不是动态收敛状态、预测信号或交易系统。
