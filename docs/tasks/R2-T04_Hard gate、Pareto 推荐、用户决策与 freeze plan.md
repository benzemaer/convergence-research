...
核心模块为 `src/r2/r2_t04_freeze_decision.py` 与 `src/r2/r2_t04_independent_validator.py`；薄入口分别位于 `scripts/r2/run_r2_t04_freeze_decision.py`、`scripts/r2/validate_r2_t04_freeze_decision.py` 和 `scripts/r2/validate_r2_t04_committed_artifacts.py`。独立 validator 必须重新读取 Phase A 文件，独立重算 cell 数、门禁归约、Pareto 行数和用户决策 pending 状态，不导入 production recommendation 逻辑。
