...
```bash
python scripts/r0/run_r0_t09_main_grid.py --input-manifest <authorized_input_manifest.json> --output-dir data/generated/r0/r0_t09/<run_id> --max-workers 6 --resume
```
...
R0-T09 contract 固化在 `configs/r0/r0_t09_main_grid_materialization_contract.v1.json`，schema 固化在 `schemas/r0/r0_t09_main_grid_materialization_contract.schema.json`。`max_workers` 默认和上限均为 6，超过 6 必须拒绝。K=1 不属于 R0-T09 confirmation grid；K=1 是 R0-T06 raw daily state reference，不作为 confirmed state materialization config。
