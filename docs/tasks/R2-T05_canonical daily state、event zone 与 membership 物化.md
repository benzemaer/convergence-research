...
状态：blocked_missing_authoritative_t04_final_gate_binding。当前未执行 formal run，也未生成 author package；T06 回放、T07 最终状态登记、T08/R3 交接不属于本 task。
...
启动复核确认 handoff 已包含 `scientific_review_status=passed`、`repository_final_gate_status=passed`、`formal_task_completed=true`、PR #96、reviewed head、workflow/job、merge commit、T04 run 和 committed input hashes；但 authoritative handoff 与 validation sidecar 均缺少 `selected_version_count`、`strict_core_only_count`。这两个计数只出现在被禁止作为启动授权依据的 pending T04 result package/freeze plan 中，因此当前启动状态必须保持 blocked。
