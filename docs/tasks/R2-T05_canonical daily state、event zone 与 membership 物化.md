...
状态：author package complete，待独立 scientific review。当前只物化 T04 selected-only 的两个 W120 primary；T06 回放、T07 最终状态登记、T08/R3 交接不属于本 task。
...
启动 handoff 已验证 `scientific_review_status=passed`、`repository_final_gate_status=passed`、`formal_task_completed=true`、`R2-T05_allowed_to_start=true`、`selected_version_count=2`、`strict_core_only_count=2`，并绑定 PR #96、reviewed head、workflow/job、merge commit、T04 run 和 committed input hashes。
