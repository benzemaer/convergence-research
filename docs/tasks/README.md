...
accepted handoff 与 canonical `DONE` 已建立。R2A-T06 尚未启动，仅在 PR #115
合并后获得启动资格。
任务契约见
...
T05 的 q20 是退出机制分解研究锚点，不是 best、optimal、selected canonical、winner 或正式参数选择。Accepted T05 不读取未来路径或收益，不定义 release、方向或强度标签，不生成交易信号或回测。T06 的 no-lookahead/PIT 要求保留为未来 release 标签协议的强制验收条件；本次 closure 不实现或启动 T06。详细契约见 [`R2A-T05_CA退出机制与跨q结构分解.md`](R2A-T05_CA退出机制与跨q结构分解.md)。
