...
D2-T20 读取 D2-T19 repaired candidate DuckDB，复制到 D2-T20 output-dir，然后在复制库内应用两类显式 policy override：三段 user-attested `listing_pause` 区间，以及 688981.SH / 689009.SH 的 `neutral_factor_1` adjustment factor policy。目标是在不伪造 daily bar、不写 provider adj_factor rows 的前提下，将剩余 coverage blocker 清零，并输出 D2 acceptance candidate report 与 D3 handoff candidate report。
...
脚本在复制库内新增 `d2_policy_listing_pause_intervals`，以 `policy_type = listing_pause`、`evidence_level = user_attested`、`applied_by_task = D2-T20` 记录。落在这些区间内的 security-date 标记为 `trading_status = listing_pause`、`daily_status = not_applicable_or_expected_empty`、`price_limit_status = not_applicable_or_expected_empty`，不再计入 listed-open missing daily 或 price-limit dependency blocker。
...
显式授权齐全且 blocker 清零时，`d2_acceptance_decision = accepted_for_d3_candidate_generation`，handoff report 可输出 `d3_candidate_generation_authorized`。即便如此，D2-T20 仍不生成 D3 rows、不发布 data_version、不生成 R0。

## 阻塞条件 / 失败状态

若缺少 `--allow-user-attested-listing-pause`、`--allow-neutral-adj-factor-policy` 或 `--authorize-d3-candidate` 任一参数，不得 accepted。若 policy 后仍有 listed-open missing daily、daily dependency、price-limit unresolved、adjustment-factor unresolved 或其他 quality blocker，不得 accepted。若 PR 生成 fake daily bar、0 price、formal data_version、D3 rows、R0 states、labels、returns、backtest 或 portfolio，PR 必须回退。

## 风险说明

三段 listing_pause 为 user-attested，不是官方公告哈希验证。688981.SH / 689009.SH 的 neutral factor 是 research candidate policy，不是 provider factor evidence。D2-T20 只用于推进 D3 research candidate；未来 formal data_version 仍需补官方证据或替代数据源验证。

## 回退方式

回退本 PR 新增脚本、任务文档、README 更新和测试即可。D2-T20 不提交 generated outputs、不修改源 D2-T19 DuckDB、不创建 manifest 或 data_version。
