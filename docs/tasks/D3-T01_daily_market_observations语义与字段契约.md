...
D3-T01 接受 refs-only canonical table 设计。D3-T01 至 D3-T06 可继续推进
contract、schema、validator、synthetic tests 和发布门禁；D3-T07 才是未来正式生成
完整 D3 data product / candidate `data_version` 的任务。当前 D3-T07 被 D2 formal
materialization 阻塞。D3-T08 只做 D3 acceptance 与 R0 handoff，不生成主数据。
...
- 不执行 D3-T07 的完整 D3 data product / candidate `data_version` 正式生成。
...
- D3 generation 未授权。
- D3-T07 blocked pending D2 formal materialization。
