...
current_sidecar_task: EXP-A04 cross-layer diagnostics
workflow_mode: long_lived_same_pr
phase: A04_cross_layer_diagnostics_implementation_review
...
formal_run_id:
execution_sha:
formal_validator_status: not_started
formal_anomaly_status: not_started
...
EXP-C01 的正式结果已由用户接受：C1-only 与 C2-only 均未达到预注册 strong-substitutability reference，保留当前 C1+C2 pair，不选择或删除指标，不创建 C v2。EXP-A 不继承 EXP-C01 的 formal-run 授权；EXP-A03 的 formal run、结果审阅与 Formal-result review 均已按独立授权完成并接受，EXP-A04 已启动 implementation review，但尚未获得 formal-run 授权。
