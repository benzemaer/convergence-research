...
current_sidecar_task: EXP-A03 candidate intralayer redundancy and selection
workflow_mode: long_lived_same_pr
phase: A03_candidate_intralayer_implementation_review
...
EXP-A03 implementation package：A1、A2、A2b 仅在三者共同 valid 的 accepted A01 key universe 上进行 raw Pearson、tie-aware midrank Spearman、年份/证券稳定性、pooled low-tail identity overlap、A2 grid 与 A2b conditional variation、variance decomposition 及预注册 provisional disposition。A03 的输入只允许 A02 accepted handoff、A02 manifest/validator/anomaly 和 A01 raw 五项 artifact；raw 只读且不复制，不创建新的持久化 DuckDB。A03 当前为 implementation review pending，未创建真实 manifest、未打开真实 raw、未执行 formal、未启动 EXP-A04，不注册 A-layer、不创建 PCATV。
...
EXP-C01 的正式结果已由用户接受：C1-only 与 C2-only 均未达到预注册 strong-substitutability reference，保留当前 C1+C2 pair，不选择或删除指标，不创建 C v2。EXP-A 不继承 EXP-C01 的 formal-run 授权；本 program 当前处于 EXP-A03 implementation review，A03 formal run 尚未授权，EXP-A04 仍未启动，下一阶段必须由用户明确授权。
