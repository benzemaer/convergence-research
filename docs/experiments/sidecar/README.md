...
EXP-C01 采用同一个 Draft PR 的两阶段流程：先审 implementation commit；只有用户明确批准当前 `reviewed_implementation_sha` 后，才允许在后续阶段执行 formal run 并提交 results / manifest / analysis。`b495201b04da9aaf1bc1b35d53586db036632489` 的批准已因日期 lineage implementation defect 失效；两个 preflight run 均在生成 artifacts 前 fail closed，当前没有正式结果，也没有创建正式 `<RUN_ID>` 结果目录。
