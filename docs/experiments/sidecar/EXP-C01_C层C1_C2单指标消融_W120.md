...
runner 会拒绝未提供批准 SHA、当前 `HEAD` 与批准 SHA 不一致、未提供精确 source manifest、输出目录已存在、manifest 声明不一致或输入无法按声明解析的运行。本次结果的工程 validator、anomaly scan、baseline reconciliation、六个 CSV 磁盘 readback 和独立统计复算均通过，并已由用户 Formal-result 审阅接受。接受结论是不批准 C1-only 或 C2-only 替代当前 pair，保留当前 C1+C2 pair；不得从本实验自动创建 C v2、删除指标或推进下一项 sidecar task。
