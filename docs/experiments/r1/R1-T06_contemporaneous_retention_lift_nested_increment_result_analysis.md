...
本 run 绑定 implementation commit `e98a529e1c828ed4b7ce9fdad24f4120717ef533`，run_id 为 `R1-T06-20260710T1155Z`。旧 run `R1-T06-20260710T1058Z` 因 nested three-valued reconciliation 和 row-level q nesting contract gap 被本 run supersede。上游 R1-T05 final package 已完成，`scientific_review_status=passed`、`anomaly_resolution_status=passed`、`downstream_gate_allowed=true`。正式输入是 repaired R0 dimension score、dimension state 和 nested daily state：score hash `4a04fbada9ecac15936e3ab5d968cba8f1205db5dbe66a0491c7141e6fc5b8a5`，state hash `bbbb49ea2056bf6f257c1821236eb2b657eb1490153dfc9e56acee8f33264e08`，nested hash `0c07f4897d76c0a729963118c2e75581bd71521a25245d6d3b650b4f32e68995`。样本范围为 20160104 至 20260630，输入 security_count 为 800。
...
q response 的 row-level nesting 与 count monotonicity checks passed，W availability monotonicity passed，同一 step/W 的 denominator 在 q=0.10/0.20/0.30 下保持一致。新增 `r1_t06_q_nesting_reconciliation.csv` 覆盖 24 条 dimension active set、18 条 anchor active set、18 条 child active set 和 18 条 denominator key-set check；所有 `missing_from_higher_q_count`、`missing_from_lower_q_count` 和 `symmetric_difference_count` 均为 0。observed_fact: q 从 0.10 到 0.30 时，baseline W250 的 retention 上升，但 Lift 下降；这说明较宽阈值提高共同出现比例的同时，也提高 target marginal rate。research_judgment: Lift 与 Delta 需要同时报告，否则会放大低 base-rate step 的相对比例。
