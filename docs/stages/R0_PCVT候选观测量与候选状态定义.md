...
| R0-T07 | `[codex] R0-T07 联合确认层、streak 与确认区间表` | `codex/r0-t07-confirmation-streak-intervals` | 必须 | 实现 `K=1/2/3` 的实时确认、streak、`raw_start_date`、`confirmation_date`、`confirmed_start_date`、终止类型和 confirmed interval 基础表。确认不得回填为早期可得信号，不做 gap merge，不生成主网格 artifact 或 manifest。 |
