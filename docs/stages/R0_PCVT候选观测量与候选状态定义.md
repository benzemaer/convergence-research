...
R0-T04 只实现这些指标的 raw/base metric engine。`AmountLevel20Pct` 的 raw base object 是 `LogAmount20`，其严格过去历史百分位和最终 `AmountLevel20Pct` 字段由 R0-T05 生成；R0-T04 不生成 percentile、score、state 或 interval。R0-T05 只生成 strict-past percentile、eligible flag、indicator score、dimension score、`score_*_min` 和 common eligible sample 语义，不应用 q 阈值、不生成 weak rule 状态、不生成 `S_PCT` / `S_PCVT` 或区间。
