...
def _read_committed_json(
    ...
    blob = _git(root, "show", f"{commit}:{rel}", binary=True)
    assert isinstance(blob, bytes)
    return blob, json.loads(blob.decode("utf-8"))
...
def main() -> int:
    ...
    try:
        _git(root, "cat-file", "-e", f"{args.artifact_commit}^{{commit}}")
        manifest_blob = _git(
            root, "show", f"{args.artifact_commit}:{manifest_rel}", binary=True
        )
        assert isinstance(manifest_blob, bytes)
        manifest_blob_sha = str(
            _git(root, "rev-parse", f"{args.artifact_commit}:{manifest_rel}")
        )
        ...
        for item in artifacts:
            try:
                ...
                blob = _git(root, "show", f"{args.artifact_commit}:{rel}", binary=True)
                assert isinstance(blob, bytes)
                blob_sha = str(_git(root, "rev-parse", f"{args.artifact_commit}:{rel}"))
    ...
    result = {
        ...
        "validated_commit": args.artifact_commit,
